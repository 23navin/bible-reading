...
- If the transcript BEGINS with a standalone announcement of a Bible reference (a reference followed by a sentence break, not woven into a sentence), remove that announcement from "text" — the app already shows the reference as a header above it. Still use it for the reference fields below. Do NOT remove the reference if removing it will make the sentence ungrammatical or nonsensical.
